    ...
    public partial class ProductManagement : Form
    {
        ...
        private void deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                // Step 2: Delete from order details (must come first due to FK constraint)
                using (SqlCommand cmd = new SqlCommand("DELETE FROM ORDER_DETAILS WHERE PID = @pid", conn))
                {
                    cmd.Parameters.AddWithValue("@pid", this.productID);
                    cmd.ExecuteNonQuery();
                }
                
                // Step 3: Delete product (must come first due to FK constraint)
